...
enum CellFragment {
    ...
    LinkBracket(&'static str, CellInlineStyle),
}
...
impl CellFragment {
    fn rendered_text(&self) -> String {
        match self {
            ...
            CellFragment::LinkBracket(b, _) => b.to_string(),
        }
    }
    ...
    fn display_width(&self) -> usize {
        ...
        match self {
            CellFragment::Text(_, _, _) | CellFragment::LinkBracket(_, _) => w,
            _ => w + 2,
        }
    }
...
pub(crate) fn handle_table_event(
    ...
    match ev {
        ...
        MdEvent::End(TagEnd::Table) => {
            let rendered = tb.render(render_width, theme);
...
pub(crate) fn start_table(table: &mut Option<TableBuf>, aligns: &[Alignment]) {
...
impl TableBuf {
    ...
    fn push_link_bracket(&mut self, b: &'static str) {
        self.current_cell
            .push(CellFragment::LinkBracket(b, self.inline_style));
    }
    fn update_link_marker_modifier(&mut self, f: impl Fn(&mut CellInlineStyle)) {
        if let Some(CellFragment::LinkBracket(_, ref mut style)) = self
            .current_cell
            .iter_mut()
            .rev()
            .find(|frag| matches!(frag, CellFragment::LinkBracket("[", _)))
        {
            f(style);
        }
    }
    ...
    pub(crate) fn render(&self, render_width: usize, theme: &MarkdownTheme) -> Vec<Line<'static>> {
...
fn align_cell(
    ...
    for frag in frags {
        match frag {
            ...
            CellFragment::LinkBracket(b, inline) => {
