...
use crate::latex;
use crate::mermaid;
use crate::tables::{handle_table_event, start_table, TableBuf};
use crate::theme::MarkdownTheme;
use crate::toc::{normalize_toc, TocEntry};
use crate::width::{display_width, line_plain_text};
use crate::wrapping::{push_wrapped_code_lines, push_wrapped_prefixed_lines};
...
fn handle_inline_style_event(
    ...
    match ev {
        ...
        MdEvent::Start(Tag::Link { .. }) => {
            inline.in_link = true;
            push_link_bracket(spans, "[", theme, *inline, blockquote_depth);
            true
        }
...
pub fn parse_markdown_with_width(
    ...
    if let Some(ref pairs) = fm_pairs {
        ...
        lines.extend(tb.render(render_width, theme_colors));
    }
