...
use common::{rendered_non_empty_lines, test_assets, test_md_theme};
use leaves::highlight::resolve_syntax;
...
use leaves::width::{build_searchable_lines, display_width, line_plain_text};
use leaves::{parse_markdown, parse_markdown_with_width};
...
fn table_inline_code_has_code_style() {
    ...
    let md = "| A |\n|---|\n| `code` |\n";
    let (lines, _) = parse_markdown(md, &ss, &theme, &test_md_theme());
    let theme_colors = MarkdownTheme::default();
...
fn mermaid_content_is_searchable() {
    ...
    let searchable = build_searchable_lines(&lines);
